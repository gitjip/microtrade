...
Product::Product(const qint64 &id, const QDateTime &createdAt, const QDateTime &removedAt,
                 const QString &name, const QString &description, double price,
                 qint64 stock, Category category, const QUrl &imageUrl)
    : Entity(id, createdAt, removedAt), m_name(name), m_description(description),
    m_price(price), m_stock(stock), m_category(category), m_imageUrl(imageUrl) {}
