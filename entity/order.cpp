...
Order::Order(const qint64 &id, const QDateTime &createdAt,
             const QDateTime &removedAt, const qint64 &userId, double cost,
             Status status)
    : Entity(id, createdAt, removedAt), m_userId(userId), m_cost(cost),
    m_status(status) {}
...
bool Order::operator<(const Order &order) const { return m_id < order.m_id; }
